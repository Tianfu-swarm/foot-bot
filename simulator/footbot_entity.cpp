...
namespace argos
{
    ...
    const CVector3 CFootBotEntity::WIFI_OFFSET_POSITION = {0.0, 0.0, 0.05};
    const Real CFootBotEntity::WIFI_TRANSMISSION_RANGE = 10.0;
    ...
    CFootBotEntity::CFootBotEntity() : CComposableEntity(nullptr),
    ...
    CFootBotEntity::CFootBotEntity(const std::string &str_id,
                                   ...
                                   Real f_perspcam_range) : CComposableEntity(nullptr, str_id),
                                                            m_pcControllableEntity(nullptr),
                                                            m_pcDistanceScannerEquippedEntity(nullptr),
                                                            m_pcTurretEntity(nullptr),
                                                            m_pcEmbodiedEntity(nullptr),
    ...
    void CFootBotEntity::Init(TConfigurationNode &t_tree)
    {
        try
        {
            ...
            AddComponent(*m_pcBatteryEquippedEntity);
            /* --- simple_radio actuator --- */
            std::string strRadioMedium;
            GetNodeAttributeOrDefault(t_tree, "radio_medium", strRadioMedium, strRadioMedium);
            Real radio_range = 5.0f;
            GetNodeAttributeOrDefault(t_tree, "radio_range", radio_range, radio_range);

            m_pcSimpleRadioEquippedEntity =
                new CSimpleRadioEquippedEntity(this, "simple_radios_0");
            if (!strRadioMedium.empty())
            {
                CSimpleRadioMedium &cMedium =
                    CSimulator::GetInstance().GetMedium<CSimpleRadioMedium>(strRadioMedium);
                static const CVector3 RADIO_OFFSET{0.0f, 0.0f, 0.10f};
                m_pcSimpleRadioEquippedEntity->AddRadio(
                    "radio",
                    RADIO_OFFSET,
                    m_pcEmbodiedEntity->GetOriginAnchor(),
                    cMedium,
                    radio_range // ← 使用新的 radio_range 变量
                );
